...
# ─── SESSION STATE INIT ───────────────────────────────────────────────────── #
for key in ["prediction_done", "prediction_data", "booking_open", "booking_confirmed"]:
    if key not in st.session_state:
        if key in ("booking_open", "booking_confirmed"):
            st.session_state[key] = {}
        elif key == "prediction_done":
...
# ─── REAL CONFUSION MATRIX (from your model screenshot) ──────────────────── #
# Rows = Actual, Columns = Predicted
# Order: No Disease, Heart Attack, Arrhythmia, Angina
REAL_CM = np.array([
    [382,   0,   0,   0],   # Actual: No Disease
    [  0, 369,   0,  13],   # Actual: Heart Attack
    [  0,   10, 407,   6],   # Actual: Arrhythmia
    [  0,  15,   0, 398],   # Actual: Angina
])

# ─── REAL CLASSIFICATION REPORT (your exact output) ──────────────────────── #
REAL_REPORT = {
...
def safe_str(s):
    """Convert any string to latin-1 safe for fpdf Helvetica font."""
    return (str(s)
        .replace("\u2013","-").replace("\u2014","--")
        .replace("\u2019","'").replace("\u2018","'")
        .replace("\u201c",'"').replace("\u201d",'"')
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not MODEL_LOADED:
        st.error(f"Model files not found. Place heart_model.pkl and label_mapping.pkl in the app folder.\n\nError: {MODEL_ERROR}")
        st.stop()
    ...
    with p1: patient_name = st.text_input("Patient Name", placeholder="e.g. Rahul Sharma", key="pname")
    with p2: age = st.slider("Age", 20, 90, 40, key="age_s")
    with p3:
        sex_label = st.selectbox("Gender",["Male","Female"], key="sex")
        sex = 1 if sex_label == "Male" else 0
    with p4: city = st.selectbox("City", CITIES, key="city")
    ...
    for row_items in [SYM[i:i+3] for i in range(0,len(SYM),3)]:
        ...
        for col,(label,key) in zip(cols,row_items):
            with col: raw_vals[key] = yn_select(label,key)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("🔮  Run Prediction", use_container_width=False):
        ...
        # Store in session_state — this means any subsequent button click
        # (booking) does NOT re-run prediction block, results persist
    ...
    if st.session_state.prediction_done and st.session_state.prediction_data:
        ...
        with r2:
            fig_g = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "steps":[{"range":[0,40],"color":"#E3F2FD"},
                              {"range":[40,70],"color":"#BBDEFB"},
                              {"range":[70,100],"color":"#90CAF9"}],
                    "threshold":{"line":{"color":"#C62828","width":3},"thickness":0.75,"value":confidence},
                },
                title={"text":"Confidence Score","font":{"size":13,"color":"#607D8B"}},
            ))
            fig_g.update_layout(height=230,margin=dict(l=20,r=20,t=30,b=10),
        ...
        with st.spinner("Generating LIME explanation..."):
            try:
                ...
                lime_exp = LimeTabularExplainer(
                    training_data=bg, feature_names=FEATURE_NAMES,
                    class_names=exp_cls_names, mode="classification", random_state=42,
                )
                pred_cls = int(prediction)
                exp = lime_exp.explain_instance(
                    input_data[0], model.predict_proba,
                    num_features=13, labels=[pred_cls],
                )
                lime_df = pd.DataFrame(exp.as_list(label=pred_cls), columns=["Feature","Weight"])
        ...
        if not city_doctors:
            ...
            for idx, doc in enumerate(city_doctors):
                ...
                # ── Book button: toggles form, does NOT reset page ──────── #
                if st.button(f"📅  Book Appointment with {doc['name']}", key=f"book_{dk}"):
                    cur = st.session_state.booking_open.get(dk, False)
                    st.session_state.booking_open[dk] = not cur
                    # Clear old confirmation when reopening
                    if not cur:
                        st.session_state.booking_confirmed.pop(dk, None)
                ...
                if st.session_state.booking_open.get(dk, False):
                    ...
                    with bf1:
                        bk_name = st.text_input("Patient Name",
                            value=patient_name or "", key=f"bname_{dk}",
                            placeholder="Full name")
                    with bf2:
                        bk_date = st.date_input("Appointment Date",
                            min_value=date.today(), key=f"bdate_{dk}")
                    with bf3:
                        bk_time = st.selectbox("Time Slot",
                    ...
                    with cc:
                        if st.button("✅  Confirm Appointment", key=f"conf_{dk}"):
                            name_d = bk_name.strip() or "Patient"
                ...
                st.markdown("<hr style='margin:10px 0;border-color:#E8EEF7'>",
                            unsafe_allow_html=True)
        ...
        try:
            ...
            def generate_pdf():
                ...
                SYM_LABELS = [
                    "Chest Pain","Shortness of Breath","Chest Tightness","Palpitations",
                    "Irregular Heartbeat","Dizziness","Fatigue","Leg Swelling",
                    "High BP","High Cholesterol","Exercise Pain",
                ]
            ...
            st.caption("Includes: patient info, prediction, symptoms, precautions, diet tips, and doctor recommendation.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Confusion Matrix (REAL values from screenshot) ─────────────────────── #
    ...
    for col,cls,color in zip([cm1,cm2,cm3,cm4], CM_LABELS,
    ...
    # ── Classification Report (REAL values) ──────────────────────────────── #
    ...
    def color_cell(val):
        try:
            v = float(val)
            if v >= 0.98: return "background-color:#C8E6C9;color:#1B5E20;font-weight:700"
            if v >= 0.95: return "background-color:#DCEDC8;color:#33691E;font-weight:600"
